...
void AppController::on_apply(){
    // En modo experto el kernel seleccionado manda; la categoría es solo informativa.
    auto name = view_.cb_kernel().get_active_text();
    Params p;
    p.k_gauss=view_.sp_gauss_k().get_value_as_int(); if(p.k_gauss%2==0) p.k_gauss+=1;
    p.k=view_.sp_k().get_value_as_int(); if(p.k%2==0) p.k+=1;
    p.sigma=view_.sp_sigma().get_value(); p.alpha=view_.sp_alpha().get_value();

    std::string err;
    bool ok = model_->apply_any(name,p,err);
    if(!ok && !err.empty()){
        Gtk::MessageDialog m(view_, err, false, Gtk::MESSAGE_ERROR, Gtk::BUTTONS_OK, true);
        m.run();
    }
    view_.refresh();
}
void AppController::on_canny(){
    ...
    if(p.t_low > p.t_high) std::swap(p.t_low, p.t_high);
